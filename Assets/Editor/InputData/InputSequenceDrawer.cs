...
public class InputSequenceDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (!_expandedStates.ContainsKey(key))
        {
            ...
            // 关键修复：在Timeline Clip中初始化默认值
            InitializeDefaultsForTimelineClip(property);
        }
        ...
        if (_expandedStates[key])
        {
            ...
            // 获取子属性
            var waitTimeProp = property.FindPropertyRelative("_waitTime");
            var dataChecksProp = property.FindPropertyRelative("_dataChecks");
    ...
    // 在Timeline Clip中初始化默认值
    private void InitializeDefaultsForTimelineClip(SerializedProperty property)
    {
        // 检查是否在Timeline Clip中
        if (property.serializedObject.targetObject is UnityEngine.Playables.PlayableAsset)
        {
            // 初始化waitTime
            var waitTimeProp = property.FindPropertyRelative("_waitTime");
            if (waitTimeProp.intValue == 0)
            {
                waitTimeProp.intValue = 40;
            }

            // 初始化dataChecks列表
            var dataChecksProp = property.FindPropertyRelative("_dataChecks");
            if (dataChecksProp.arraySize == 0)
            {
                // 添加一个默认元素
                dataChecksProp.arraySize = 1;
                var element = dataChecksProp.GetArrayElementAtIndex(0);
                var dataCheckProp = element.FindPropertyRelative("dataCheck");

                // 创建新的ButtonInputCondition实例
