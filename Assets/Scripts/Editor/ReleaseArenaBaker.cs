...
public static class ReleaseArenaBaker
{
    ...
    private static void BuildEnvironmentInScene(Scene scene)
    {
        ...
        matFire.SetColor("_EmissionColor", new Color(1.0f, 0.32f, 0.05f) * 1.7f);

        // Clean, flat boss-combat space. Top surface is y = 0.
        CreateCube("Ground_Base_Foundation", geometry.transform, new Vector3(0f, -0.15f, 0f), new Vector3(30f, 0.3f, 30f), matDarkStone, GroundTag, "Ground", true);
        CreateOctagonalPrism("Ground_MainArena_Octagon", geometry.transform, 12f, 0.28f, new Vector3(0f, 0.02f, 0f), matFloor, GroundTag, "Ground");
        CreateCube("Ground_CenterMark_Visual", geometry.transform, new Vector3(0f, 0.175f, 0f), new Vector3(5.4f, 0.04f, 5.4f), matAccent, GroundTag, "Ground", false);

        // Camera-safe hard boundaries. These are large, simple colliders.
        CreateCube("Obstacle_Boundary_NorthWall", geometry.transform, new Vector3(0f, 2.5f, 14.5f), new Vector3(30f, 5f, 1f), matWall, ObstacleTag, "Obstacle", true);
        CreateCube("Obstacle_Boundary_SouthWall", geometry.transform, new Vector3(0f, 2.5f, -14.5f), new Vector3(30f, 5f, 1f), matWall, ObstacleTag, "Obstacle", true);
        CreateCube("Obstacle_Boundary_EastWall", geometry.transform, new Vector3(14.5f, 2.5f, 0f), new Vector3(1f, 5f, 30f), matWall, ObstacleTag, "Obstacle", true);
        CreateCube("Obstacle_Boundary_WestWall", geometry.transform, new Vector3(-14.5f, 2.5f, 0f), new Vector3(1f, 5f, 30f), matWall, ObstacleTag, "Obstacle", true);

        // Ceiling exists for systems that query Floor above the combat space. It is transparent visual-wise.
        var ceiling = CreateCube("Floor_Ceiling_CombatLimit", geometry.transform, new Vector3(0f, 8f, 0f), new Vector3(30f, 0.35f, 30f), matDarkStone, FloorTag, "Floor", true);
        ...
        CreateCube("Obstacle_WallCombo_Backplate", geometry.transform, new Vector3(0f, 2.8f, 13.15f), new Vector3(12f, 5.6f, 0.8f), matWall, ObstacleTag, "Obstacle", true);
        CreateCube("Obstacle_WallCombo_LeftBreak", geometry.transform, new Vector3(-5.6f, 4.1f, 12.75f), new Vector3(2.2f, 2.4f, 0.9f), matDarkStone, ObstacleTag, "Obstacle", true);
        CreateCube("Obstacle_WallCombo_RightBreak", geometry.transform, new Vector3(5.6f, 3.6f, 12.75f), new Vector3(2.2f, 3.0f, 0.9f), matDarkStone, ObstacleTag, "Obstacle", true);
        ...
        CreateCube("Dressing_BrokenGate_TopVisual", dressing.transform, new Vector3(0f, 6f, 15.1f), new Vector3(9.5f, 1.2f, 0.8f), matDarkStone, "Untagged", "Default", false);
        CreateCube("Dressing_BrokenGate_LeftVisual", dressing.transform, new Vector3(-5.2f, 3.0f, 15.1f), new Vector3(1.2f, 6f, 0.8f), matDarkStone, "Untagged", "Default", false);
        CreateCube("Dressing_BrokenGate_RightVisual", dressing.transform, new Vector3(5.2f, 3.0f, 15.1f), new Vector3(1.2f, 6f, 0.8f), matDarkStone, "Untagged", "Default", false);
    ...
    private static GameObject CreateCube(string name, Transform parent, Vector3 position, Vector3 scale, Material material, string tag, string layerName, bool colliderEnabled)
    {
    ...
    private static GameObject CreateOctagonalPrism(string name, Transform parent, float radius, float height, Vector3 position, Material material, string tag, string layerName)
    {
        var go = new GameObject(name);
        go.transform.SetParent(parent, false);
        go.transform.position = position;
    ...
    private static void CreateVisualPillar(string name, Transform parent, Vector3 position, Material material)
    {
        ...
        CreateCube(name + "_Base", root.transform, position + new Vector3(0f, -1.75f, 0f), new Vector3(1.6f, 0.5f, 1.6f), material, "Untagged", "Default", false);
        CreateCube(name + "_Shaft", root.transform, position + new Vector3(0f, 0f, 0f), new Vector3(1.0f, 4.0f, 1.0f), material, "Untagged", "Default", false);
        CreateCube(name + "_Cap", root.transform, position + new Vector3(0f, 2.2f, 0f), new Vector3(1.8f, 0.5f, 1.8f), material, "Untagged", "Default", false);
    }
    ...
    private static void CreateBrazier(string name, Transform visualParent, Vector3 position, Material stoneMaterial, Material fireMaterial, Transform lightParent)
    {
        ...
        root.transform.position = position;
        CreateCube(name + "_StoneBase", root.transform, position, new Vector3(1.2f, 0.7f, 1.2f), stoneMaterial, "Untagged", "Default", false);
        CreateCube(name + "_FireGlow", root.transform, position + Vector3.up * 0.55f, new Vector3(0.65f, 0.65f, 0.65f), fireMaterial, "Untagged", "Default", false);
    ...
    private static void CreateDebugMarker(string name, Transform parent, Vector3 position, Vector3 scale)
    {
        ...
        var go = CreateCube(name, parent, position, scale, material, "Untagged", "Default", false);
        var renderer = go.GetComponent<Renderer>();
        if (renderer != null)
    ...
    private static void EnsureTagsAndLayers()
    {
        ...
        EnsureTag(tagManager, GroundTag);
        EnsureTag(tagManager, FloorTag);
        EnsureTag(tagManager, ObstacleTag);
        EnsureLayer(tagManager, 8, GroundTag);
        EnsureLayer(tagManager, 9, FloorTag);
        EnsureLayer(tagManager, 10, ObstacleTag);
